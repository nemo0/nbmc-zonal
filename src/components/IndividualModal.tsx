...
export default function CamperModal(props: Props) {
  const { isOpen, onRequestClose, row } = props;
  const [camper, setCamper] = React.useState<IEditCamper>({} as any);
  ...
  React.useEffect(() => {
    if (row) {
      ...
      Object.keys(row).forEach((key) => {
        setValue(key as keyof IIndividualCamper, row[key]);
      });
    }
  }, [row, setValue]);
