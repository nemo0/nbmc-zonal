...
import RegistrationPreviewModal, {
  PreviewSection,
} from '@/components/RegistrationPreviewModal';
...
export default function OrganizationForm() {
  ...
  const [previewData, setPreviewData] = useState<IOrganizationEntry | null>(
    null
  );
  ...
  const previewSections: PreviewSection[] = previewData
    ? [
        {
          ...
          fields: [
            ...
            {
              label: 'Unit/Organization Address',
              value: previewData.organizationAddress,
            },
            {
              label: 'Unit/Organization Email Address',
              value: previewData.organizationEmail,
            },
            {
              label: 'Unit/Organization Contact Number',
              value: previewData.organizationContact,
            },
            {
              label: 'Unit/Organization Contact Person Name',
              value: previewData.organizationContactPerson,
            },
          ],
        },
        ...previewData.campers.map((camper, index) => ({
          title: `Camper ${index + 1}`,
          fields: [
            { label: 'Name', value: camper.name },
            { label: "Father/Mother's Name", value: camper.guardian },
            { label: 'Address', value: camper.address },
            { label: 'District', value: camper.district },
            { label: 'Pin Code', value: camper.pin },
            { label: 'Age', value: camper.age },
            { label: 'Occupation', value: camper.occupation },
            { label: 'Class/Course with Year', value: camper.course },
            { label: 'Contact Number', value: camper.contact },
            { label: 'Email', value: camper.email },
            { label: 'Physically Fit', value: camper.physicallyFit },
            { label: 'Religion', value: camper.religion },
            { label: 'Food Preference', value: camper.foodPreference },
            { label: 'Camp Experience', value: camper.campExperience },
            { label: 'Nature of Camper', value: camper.natureOfCamper },
            { label: 'Amount', value: camper.amount },
          ],
        })),
      ]
    : [];

  const submitRegistration = async (data: IOrganizationEntry) => {
    try {
      setLoading(true);
  ...
  const openPreview: SubmitHandler<IOrganizationEntry> = (data) => {
    setPreviewData(data);
  ...
  const handleConfirmSubmit = async () => {
    ...
    await submitRegistration(previewData);
  };

  return (
    <>
      <form onSubmit={handleSubmit(openPreview)}>
        <div>
            ...
            <div className='' key={index}>
              ...
              <div className='form-group'>
                ...
                <select
                  ...
                  key={item.id}
                  {...register(`campers.${index}.occupation`, {
                    required: true,
                  })}
                >
                  <option value='' disabled></option>
      ...
      <RegistrationPreviewModal
        ...
        title='Review Organization Registration'
      />
    </>
  );
}
